...
/*
    一：这节 分析HashSet的扩容和转成红黑树机制

    二、结论：
    1. HashSet底层是HashMap，第一次添加时，table数组扩容到16，临界值(threshold)时16*临界因子(loadFactor)
    时0.75=12
    2. 如果每个数组的链表中占用总量达到临界值12，就扩容到16*2=32，新的临界值就是32*0.75=24，以此类推
    3. Java8中，如果一条链表的元素个数到达8个且table的大小>=64，该条链表就会进行树化（红黑树），否则仍然采用数组扩容
    机制。【如果一条链表的元素个数到达8个，就会进行table的扩容】
 */
...
class A {
    ...
    @Override
    public int hashCode() { //这里修改hashCode()方法，h = key.hashCode()得到的就是一样的了
        return 100;
    }
}