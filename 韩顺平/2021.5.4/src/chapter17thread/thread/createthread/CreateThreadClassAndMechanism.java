...
/*
    ...
    6. start0()方法才是实现多线程的方法，而不是run().又因为start0()是由start()方法调用的，所以用cat.start();来启动线程
    start()底层会创建新的线程(有矛盾:前面说 3. 主线程中，创建了一个子线程，Cat cat = new Cat();)，
    调用run(),run()就是一个简单的方法调用，不能够启动新线程
    7. main线程的子线程会单独留下来执行，main线程中的子线程不算main线程的进度。所以main线程很早就退出了，而子线程还在继续
...
class Cat extends Thread {
    ...
    public void run() {
        ...
        while (true) {
            ...
            System.out.println("Cat线程名称=" + Thread.currentThread().getName()+"喵喵, 我是小猫咪" + (++times));
            try {
                //让该线程休眠1秒
                Thread.sleep(1000);// 这里会报编译错误，可以try-catch来提前处理，主方法中的sleep异常可以用throws的方法处理
