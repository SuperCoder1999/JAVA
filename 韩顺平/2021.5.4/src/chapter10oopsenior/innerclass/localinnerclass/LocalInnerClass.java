...
/*
  一：演示局部内部类的使用细节
  1. 局部内部类可以直接访问外部类的所有成员，包含私有的。访问当前方法的局部变量或形参，则变量必须用final修饰，也就是必须是常量。
  ...
  4. 外部类访问局部内部类的成员方式：能间接访问，也只限于方法体内
...
class Outer02 {//外部类
    ...
    public void m1(final int n5) {//方法
        int n2 = 200; //Java8中自动设置为effectively final。不需要手动定义final
        int n2_2 = 200; //Java8中自动设置为effectively final。不需要手动定义final
        ...
        final class Inner02 {//局部内部类(本质仍然是一个类)
            ...
            public void f1() {
                ...
                System.out.println("使用局部变量n2=" + n2);
                System.out.println("使用形参变量n5=" + n5);
                n3 = 123;
                m2();
            }
...
//Inner02本质就是类，可以在所在”局部范围“内被另一个局部内部类中继承；”局部范围“外的类无法继承内部类
...
//class Other02 extends Inner02{//”局部范围“外的类无法使用内部类
//
//}